...
#### Storage Blob: Prefer `BlobServiceClient` entry point
...
let credential = DeveloperToolsCredential::new(None)?;
let service_url = "https://<storage_account_name>.blob.core.windows.net/".parse()?;
...
#### Cosmos DB: Builder pattern with `AccountReference`
...
// Key Vault Secrets/Keys/Certificates — iterate items directly from pager
